...
describe("SkillScanner", () => {
  ...
  describe("scan", () => {
    ...
    it("should handle scan errors gracefully", async () => {
      const result = await scanner.scan("/nonexistent/directory");
    ...
    it("should handle multiple valid skills", async () => {
      const skill1Dir = resolve(testDir, "skill1");
      const skill2Dir = resolve(testDir, "skill2");
      const skill3Dir = resolve(testDir, "skill3");
      ...
      expect(result.artifacts).toHaveLength(3);
      const skillNames = result.artifacts.map((a) => a.id).sort();
