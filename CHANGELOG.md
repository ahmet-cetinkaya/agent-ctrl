...
- Apply agents via pi-subagents when detected
- Apply MCP servers via pi-mcp-adapter when detected
- Add Pi platform adapter

### Fixed

- Address Pi platform code-review findings
- Address Pi platform review findings

## [0.9.0] - 2026-09-21

### Added

- Resolve model frontmatter per platform
- Add model frontmatter resolver and transformer

### Fixed

- Address model frontmatter review findings
...
- Normalize agent tools array into boolean map (#10)

## [0.7.0] - 2026-07-12

### Added

- Show grouped metadata in list and apply

### Fixed

- Surface metadata read errors and tighten DI/type safety

### Added

- Show profile metadata (name, description, tags) grouped by category in `profile list` and the interactive `profile apply` picker, sourced from an optional `profile.yaml` per profile directory

### Fixed

- Surface a warning instead of silently swallowing a malformed or unreadable `profile.yaml`, rather than treating it the same as a missing file
...
- Read version from package.json instead of hardcoded literal
...
- Add support for installing skills via git references
...
[unreleased]: https://github.com/ahmet-cetinkaya/agent-ctrl/compare/v0.10.0...HEAD
[0.10.0]: https://github.com/ahmet-cetinkaya/agent-ctrl/releases/tag/v0.10.0
[0.9.0]: https://github.com/ahmet-cetinkaya/agent-ctrl/releases/tag/v0.9.0
