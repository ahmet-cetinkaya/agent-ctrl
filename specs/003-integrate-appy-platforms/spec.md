# Feature Specification: Integrate Appy Per Platform

**Feature Branch**: `003-integrate-appy-platforms`  
**Created**: 2026-03-06  
**Status**: Draft  
**Input**: User description: "agent-ctrl should be configured to integrate the appy command across OpenCode, Gemini, Qwen, Kilo, Antigravity, Codex, Cursor, and Windsurf platforms by applying configurations for each"

## Clarifications

### Session 2026-03-06

- Q: How should the system handle conflicting existing `appy` entries on a platform? → A: Replace conflicting existing `appy` entries with the required valid `appy` configuration automatically.
...
### User Story 1 - Apply Appy To A Selected Platform (Priority: P1)

As a CLI operator, I can run the apply workflow for a selected platform (for example, `agent-ctrl apply opencode`) to configure the `appy` integration for that platform using its supported configuration model.

**Why this priority**: This is the core business value: one command reliably configures `appy` for the intended platform target.

**Independent Test**: Can be fully tested by running apply for one platform on a project without prior setup and verifying that exactly that platform receives a valid managed `appy` integration artifact.

**Acceptance Scenarios**:

1. **Given** a project with no existing `appy` integration for OpenCode, **When** the operator runs `agent-ctrl apply opencode`, **Then** the system applies `appy` configuration for OpenCode and reports OpenCode success.
2. **Given** a project where `appy` is already configured for OpenCode, **When** the operator runs `agent-ctrl apply opencode`, **Then** the system preserves valid existing settings and avoids duplicate entries.
3. **Given** a project where `appy` is not configured for Gemini, **When** the operator runs `agent-ctrl apply gemini`, **Then** the system applies `appy` configuration only for Gemini.
4. **Given** a project where `appy` is not configured for Qwen, **When** the operator runs `agent-ctrl apply qwen`, **Then** the system applies `appy` configuration only for Qwen.
5. **Given** a project where `appy` is not configured for Kilo, **When** the operator runs `agent-ctrl apply kilo`, **Then** the system applies `appy` configuration only for Kilo.
6. **Given** a project where `appy` is not configured for Antigravity, **When** the operator runs `agent-ctrl apply antigravity`, **Then** the system applies `appy` integration only for Antigravity using the documented Antigravity customization surface.
7. **Given** a project where `appy` is not configured for Codex, **When** the operator runs `agent-ctrl apply codex`, **Then** the system applies `appy` integration only for Codex using the documented Codex configuration/customization surface.
8. **Given** a project where `appy` is not configured for Cursor, **When** the operator runs `agent-ctrl apply cursor`, **Then** the system applies `appy` integration only for Cursor using documented Cursor rule/configuration surfaces.
9. **Given** a project where `appy` is not configured for Windsurf, **When** the operator runs `agent-ctrl apply windsurf`, **Then** the system applies `appy` integration only for Windsurf using documented Windsurf rule/workflow surfaces.
...
- Existing configuration contains malformed `appy` entries that require replacement to reach a valid desired state.
- A platform name is invalid or unsupported in the command input.
- The command is run without a platform argument.
- Platform-specific configuration sources differ in structure, but each supported platform still needs an equivalent valid `appy` integration outcome when targeted.
...
- **FR-003**: System MUST configure the `appy` integration only for the selected platform target during an execution.
- **FR-004**: System MUST validate whether the selected platform already contains the required `appy` configuration before writing changes.
- **FR-005**: System MUST create missing selected-platform configuration entries required for `appy` integration.
- **FR-006**: System MUST automatically replace invalid, outdated, or conflicting existing `appy` platform entries with the required valid state.
- **FR-007**: System MUST avoid duplicate `appy` entries when run repeatedly for the same selected platform.
- **FR-008**: System MUST preserve non-`appy` platform settings and unrelated user-defined configuration.
...
- **FR-012**: System MUST support rerun behavior that converges selected-platform configuration to the same desired `appy` integration state.
...
- **FR-015**: System MUST use the selected platform’s documented customization surface for `appy` integration and MUST NOT rely on undocumented/private configuration mechanisms.
...
- **FR-017**: For Antigravity, Cursor, and Windsurf (where command-file customization is not the primary documented interface), system MUST represent `appy` integration through documented rule/workflow/skill-style customization surfaces.
...
- **FR-019**: For Cursor, system MUST preserve compatibility across project, user, and team rule scopes when applying `appy` integration.
- **FR-020**: For Windsurf, system MUST preserve compatibility across workspace and global rule/workflow scopes when applying `appy` integration.
...
- **Appy Integration Artifact**: The required platform-native integration representation (for example command definition or rule/workflow artifact) that represents a valid `appy` setup for a platform.
...
- All eight target platforms expose a documented customization surface that can represent equivalent `appy` integration behavior.
...
- Supported customization-surface references are baselined in `specs/003-integrate-appy-platforms/research.md` under "Cross-Platform Configuration Surface Analysis (2026-03-07)".
...
- **SC-003**: Re-running apply after a successful run for the same platform yields no duplicate `appy` entries.
